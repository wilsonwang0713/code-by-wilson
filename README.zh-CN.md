...
侧栏顶部直接从 `.claude` 目录读出你的账户。订阅账户（Pro 或 Max）下，它显示你的套餐和限流量
表，附实时的重置倒计时，让你看清自己离上限还有多远。API 账户下，它显示接口主机和套餐。账户邮箱
默认打码，点一下即可显示，方便你共享屏幕时不泄露它。

**CLI 准备好了吗？** 账户下方有一条状态带，检查你本地的 Claude Code：是否找得到、是否够新、
是否已登录，没就绪时给你指出怎么修。
