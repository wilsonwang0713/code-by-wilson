...
const cache = new Map<string, { gitDir: string | null; token: string; expiry: number; value: GitInfo | null }>()
...
export function readGit(cwd: string): GitInfo | null {
  ...
  if (gitDir === null) {
    cache.set(cwd, { gitDir: null, token: '', expiry: now + TTL_MS, value: null })
    return null
  }
