...
import { formatRelativeTime, costDisplay } from '@shared/format'
import { STATE_META, ctxTone, ctxBar } from './ui/meta'
...
export function Overview({ sessions, account, loading, onRefresh, onOpen, onNew }: Props) {
    ...
    <div className="app-bg flex h-screen flex-col text-fg">
      <header className="flex shrink-0 items-center gap-4 border-b border-ink-800 px-5 py-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            onClick={onNew}
            className="shrink-0 rounded-md bg-primary px-3 py-1.5 text-xs font-semibold text-ink-950 transition-colors hover:bg-primary-bright"
          >
            ＋ New session
          </button>
          <button
            onClick={onRefresh}
            disabled={loading}
            className="shrink-0 rounded-md border border-ink-700 bg-ink-800 px-3 py-1.5 text-xs text-fg transition-colors hover:bg-ink-750 disabled:cursor-default disabled:opacity-60"
          >
