...
export function TokensPanel({ usage }: { usage: Usage }) {
  ...
  return (
    <PanelSection>
      <PanelHeading
        info={TOKENS_INFO}
        right={<span className="font-mono text-[12px] tabular-nums text-fg-muted">{formatTokensShort(total)}</span>}
      >
        Tokens
      </PanelHeading>
      <StackedBar className="mt-1" segments={legend.map((l) => ({ value: l.value, color: l.color }))} />
