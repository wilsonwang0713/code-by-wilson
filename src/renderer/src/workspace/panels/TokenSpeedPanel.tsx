  ...
  return (
    <PanelSection>
      ...
      <div className="mt-1 flex items-baseline justify-between">
        ...
        <span className="font-mono text-base font-bold tabular-nums text-primary">{formatTps(speed.totalTps)}</span>
      </div>
      <RateBar label="Output" value={formatTps(speed.outputTps)} pct={ratePct(speed.outputTps, max)} color="var(--color-primary)" />
      <RateBar label="Input" value={formatTps(speed.inputTps)} pct={ratePct(speed.inputTps, max)} color="var(--color-fg-muted)" />
    </PanelSection>
  )
}
