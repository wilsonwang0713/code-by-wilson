...
export interface PaneStateSnapshot {
  ...
  /** Vertical size override (px) for panes that resize on the Y axis (a bottom-row pane). */
...
function isSnapshot(value: unknown): value is PaneStateSnapshot {
  ...
  const widthOk =
    r.widthOverride === undefined ||
    (typeof r.widthOverride === "number" && Number.isFinite(r.widthOverride));
  const heightOk =
    r.heightOverride === undefined ||
    (typeof r.heightOverride === "number" && Number.isFinite(r.heightOverride));
...
// Persists both open state and resize sizes; load() validates each snapshot.
...
const stateCache = new Map<
  string,
  ReadableAtom<PaneStateSnapshot | undefined>
>();
...
export const $paneOpen = (id: string) =>
  memoized(openCache, id, (s) => s?.open ?? false);
export const $paneState = (id: string) => memoized(stateCache, id, (s) => s);
export const $paneWidthOverride = (id: string) =>
  memoized(widthCache, id, (s) => s?.widthOverride);
export const $paneHeightOverride = (id: string) =>
  memoized(heightCache, id, (s) => s?.heightOverride);

export function ensurePaneRegistered(
  id: string,
  defaults: PaneRegisterDefaults,
): void {
...
export function togglePane(id: string): void {
  ...
  $paneStates.set({
    ...current,
    [id]: { ...existing, open: !(existing?.open ?? false) },
  });
}

export function setPaneWidthOverride(
  id: string,
...
export function setPaneHeightOverride(
  id: string,
  height: number | undefined,
): void {
  const current = $paneStates.get();
  const existing = current[id] ?? { open: false };
  if (existing.heightOverride === height) return;
  $paneStates.set({
    ...current,
    [id]: { ...existing, heightOverride: height },
  });
}

export const clearPaneWidthOverride = (id: string) =>
  setPaneWidthOverride(id, undefined);
export const clearPaneHeightOverride = (id: string) =>
