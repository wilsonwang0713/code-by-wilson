...
/** Square hit-target size (CSS px) of one cluster button. */
export const TITLEBAR_CONTROL_SIZE = 28;
...
export const TITLEBAR_CONTROLS_TOP =
  (HEADER_HEIGHT_PX - TITLEBAR_CONTROL_SIZE) / 2;
 ...
 *  controls.left + control size + half a control of breathing room. Deliberately NOT animated —
...
export function titlebarContentInsetPx(
  ...
  return (
    ...
    Math.round(TITLEBAR_CONTROL_SIZE / 2)
  );
}
...
export function headerRightPaddingPx(rightClusterOverHeader: boolean): number {
  return rightClusterOverHeader
    ? TITLEBAR_EDGE_INSET + TITLEBAR_CONTROL_SIZE + 8
    : HEADER_EDGE_PADDING_PX;
}
