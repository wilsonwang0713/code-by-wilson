...
describe('createStatusLineReader — new fields', () => {
  it('parses version, effort, cwd, session clock, and the weekly sub-buckets', () => {
    ...
    writeCapture(home, 'sid-1', {
      ...
      rate_limits: {
        ...
        seven_day_sonnet: { used_percentage: 52, resets_at: NOW / 1000 + 273_600 },
        seven_day_opus: { used_percentage: 81, resets_at: NOW / 1000 + 273_600 },
      },
    })
    const [s] = createStatusLineReader({ claudeDir: home, now: () => NOW }).read()
  ...
  it('reads cwd from workspace.current_dir when top-level cwd is absent', () => {
    const home = makeHome()
    writeCapture(home, 'sid-3', { session_id: 'sid-3', workspace: { current_dir: '/ws/dir' } })
    const [s] = createStatusLineReader({ claudeDir: home, now: () => NOW }).read()
