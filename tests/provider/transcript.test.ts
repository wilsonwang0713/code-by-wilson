...
const fx = (p: string) =>
  readFileSync(resolve("tests/fixtures/claude-home", p), "utf8");
...
describe("parseTranscript", () => {
  ...
  it("derives a title from array-form user content (content blocks)", () => {
    const jsonl = JSON.stringify({
      ...
      message: {
        role: "user",
        content: [{ type: "text", text: "Add pagination to the list view" }],
      },
    });
    expect(parseTranscript(jsonl).title).toBe(
    ...
    const jsonl = [
      {
        ...
        isMeta: false,
        cwd: "/work/app",
        message: { role: "user", content: "Summarize the file" },
      },
      {
        type: "assistant",
        message: {
  ...
  it("reflects only the latest assistant turn, not an earlier abandoned tool_use", () => {
    ...
    const jsonl = [
      ...
      },
      {
        type: "user",
        isMeta: false,
        message: { role: "user", content: "stop, read this first" },
      },
      {
        type: "assistant",
        message: {
          role: "assistant",
          content: [
            { type: "tool_use", id: "toolu_new", name: "Read", input: {} },
          ],
        },
      },
      {
...
describe("firstTranscriptCwd", () => {
  ...
  it("returns empty string when no row resolves a cwd", () => {
    expect(
      firstTranscriptCwd('{"type":"user","message":{"content":"hi"}}'),
    ).toBe("");
...
describe("deriveTitle", () => {
  ...
  it("skips empty prompts and picks the first real one", () => {
    expect(
      deriveTitle(
        ["<command-message></command-message>", "   ", "Fix the timeout bug"],
        "/x/y",
      ),
