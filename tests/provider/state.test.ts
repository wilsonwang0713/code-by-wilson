...
describe("deriveSessionState", () => {
  it("is working when alive and generating", () => {
    expect(
      deriveSessionState({ alive: true, status: "busy", awaitingUser: false }),
    ).toBe("working");
  });

  it("is waiting when alive, quiet, and blocked on an unanswered prompt", () => {
    expect(
      deriveSessionState({ alive: true, status: "idle", awaitingUser: true }),
    ).toBe("waiting");
  });
  ...
  it("is idle when alive, quiet, and the last turn is finished", () => {
    expect(
      deriveSessionState({ alive: true, status: "idle", awaitingUser: false }),
    ).toBe("idle");
  });

  it("is ended when the process is gone", () => {
    expect(
      deriveSessionState({ alive: false, status: "idle", awaitingUser: false }),
    ).toBe("ended");
  });
  ...
  it("resolves precedence: gone beats all, generating beats an unanswered prompt", () => {
    ...
    expect(
      deriveSessionState({ alive: false, status: "busy", awaitingUser: true }),
    ).toBe("ended");
    // A live, generating session is Working even with a tool mid-flight (not blocked on us).
    expect(
      deriveSessionState({ alive: true, status: "busy", awaitingUser: true }),
    ).toBe("working");
