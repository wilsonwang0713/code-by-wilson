...
function writeSubagent(home: string, proj: string, id: string, agentId: string, meta: object, body: string): void {
...
describe('readTranscript — subagents', () => {
  it('attaches the reconstructed forest from the subagents dir', () => {
    const home = makeHome()
    const id = 'sid-1'
    writeMain(home, '-work-x', id, jsonl(
      { type: 'assistant', message: { content: [{ type: 'tool_use', id: 'tu-1', name: 'Task' }] } },
      { type: 'user', message: { content: [{ type: 'tool_result', tool_use_id: 'tu-1', is_error: false }] } },
    ))
    writeSubagent(home, '-work-x', id, 'a1', { agentType: 'Explore', description: 'd', toolUseId: 'tu-1' }, jsonl(
      { type: 'assistant', timestamp: '2026-06-04T03:00:00.000Z', message: { model: 'global.anthropic.claude-sonnet-4-6', usage: { input_tokens: 4, output_tokens: 20 }, content: [] } },
      { type: 'assistant', timestamp: '2026-06-04T03:00:04.000Z', message: { model: 'global.anthropic.claude-sonnet-4-6', usage: { input_tokens: 1, output_tokens: 5 }, content: [] } },
    ))
    ...
    expect(r.doc.subagents).toEqual([
      { id: 'a1', type: 'Explore', status: 'done', model: 'claude-sonnet-4-6', tokens: 30, durationMs: 4000 },
    ])
  ...
  it('returns an empty forest when there is no subagents dir', () => {
    const home = makeHome()
    writeMain(home, '-work-y', 'sid-2', jsonl({ type: 'assistant', message: { content: [{ type: 'text', text: 'hi' }] } }))
