...
import { glyphClass, glyphTitle, glyphPulses } from '../../src/renderer/src/ui/session-glyph'
...
describe('glyphClass — color = state, fill = management', () => {
  ...
  it('is a hollow ring in the same state color for an observed session', () => {
    expect(glyphClass('waiting', 'observed')).toBe('border-[1.5px] bg-transparent border-accent')
    expect(glyphClass('working', 'observed')).toBe('border-[1.5px] bg-transparent border-working')
    expect(glyphClass('ended', 'observed')).toBe('border-[1.5px] bg-transparent border-ink-600')
  })
})
