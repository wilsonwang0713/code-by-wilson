...
import { donutGradient, ringGradient, segmentPercents, ratePct } from '../../src/renderer/src/ui/charts-geom'
...
describe('ringGradient', () => {
  it('fills up to pct, then the track', () => {
    expect(ringGradient(64, 'FILL', 'TRACK')).toBe('conic-gradient(FILL 0% 64%, TRACK 64% 100%)')
  })

  it('clamps over 100 and under 0', () => {
    expect(ringGradient(150, 'F', 'T')).toBe('conic-gradient(F 0% 100%, T 100% 100%)')
    expect(ringGradient(-5, 'F', 'T')).toBe('conic-gradient(F 0% 0%, T 0% 100%)')
  })
})

describe('donutGradient', () => {
  it('lays segments end to end, the last ending exactly at 100%', () => {
    expect(donutGradient([{ value: 1, color: 'A' }, { value: 3, color: 'B' }], 'T')).toBe(
      'conic-gradient(A 0% 25%, B 25% 100%)',
    )
  })

  it('handles three segments', () => {
    expect(
      donutGradient([{ value: 1, color: 'A' }, { value: 1, color: 'B' }, { value: 2, color: 'C' }], 'T'),
    ).toBe('conic-gradient(A 0% 25%, B 25% 50%, C 50% 100%)')
  })

  it('rounds intermediate boundaries to two decimals', () => {
    expect(
      donutGradient([{ value: 1, color: 'A' }, { value: 1, color: 'B' }, { value: 1, color: 'C' }], 'T'),
