...
function turnRows(userTs: string, asstTs: string, id: string, input: number, output: number) {
  return [
...
describe('computeTokenSpeed', () => {
  it('rates tokens over the active interval (user→assistant)', () => {
    const rows = turnRows('2026-06-11T00:00:00.000Z', '2026-06-11T00:00:10.000Z', 'm1', 200, 1000)
  ...
  it('counts each assistant message id once (multi-block turns are not multiplied)', () => {
    const rows = [
      { type: 'user', timestamp: '2026-06-11T00:00:00.000Z', message: { content: 'hi' } },
      { type: 'assistant', timestamp: '2026-06-11T00:00:10.000Z', message: { id: 'm1', usage: { input_tokens: 200, output_tokens: 1000 } } },
      { type: 'assistant', timestamp: '2026-06-11T00:00:10.000Z', message: { id: 'm1', usage: { input_tokens: 200, output_tokens: 1000 } } },
    ]
  ...
  it('clips to the rolling window anchored at the last activity', () => {
    const rows = [
      ...turnRows('2026-06-11T00:00:00.000Z', '2026-06-11T00:00:10.000Z', 'old', 0, 1000),
      ...turnRows('2026-06-11T00:01:40.000Z', '2026-06-11T00:01:50.000Z', 'new', 0, 500),
    ]
  ...
  it('returns null with no completed request or zero active duration', () => {
    ...
    const instant = turnRows('2026-06-11T00:00:00.000Z', '2026-06-11T00:00:00.000Z', 'm1', 10, 10)
